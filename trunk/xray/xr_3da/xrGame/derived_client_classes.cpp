...
bool is_sprint_allowed(const CInventoryItem *I)  { return !!I->IsAllowSprint(); }
void set_sprint_allowed(CInventoryItem *I, bool yep)
{
	I->m_flags.set(CInventoryItem::FAllowSprint, yep);
}
...
void CInventoryScript::script_register(lua_State *L)
{
	module(L)
		[
			...
			.property("sprint_allowed"					,			&is_sprint_allowed, &set_sprint_allowed)
...
void COutfitScript::script_register(lua_State *L)
{
	module(L)
		[
			...
			.def_readwrite("artefact_count"							        ,		&CCustomOutfit::m_artefact_count)
